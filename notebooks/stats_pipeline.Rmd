...
### Random effects
`subject`, `item` and `question` are coded to have random intercepts. This means that, the particular subject, item or question may differentially effect reaction times. Additionally, random slopes are coded for `item` and `question`. This means that each subject might vary in their responses to different items and questions.

<div class="alert alert-warning">
  <strong>Note:</strong> The model with the interaction term continues to outperform that without. I am convinced there is an issue with the data generation process but I am still troubleshooting. As you can see, there is clearly no interaction between modality and question in the data (see the plots in the data generation notebook). I will update accordingly. For now, these models are merely a skeleton.
</div>

```{r}
shared <- lmer(rt ~ modality + question + (1 + item + question|subject) + (1|item) + (1|question) , data = df, REML = FALSE) # nolint
separate <- lmer(rt ~ modality * question + (1 + item + question|subject) + (1|item) + (1|question), data = df, REML = FALSE) # nolint
```
